...
function SignupTab() {
  ...
  const textNameInputChange = (val: string) => {
    //console.log('textchange');
    const reg = /\S+@\S+/;
    if (reg.test(val) === true) {
      setData({
  ...
  const signinActivity = async () => {
    try {
      ...
      const response = await fetch(
        "https://zjil8ive37.execute-api.ca-central-1.amazonaws.com/dev/wm/signup",
        {
          method: "POST",
          headers: {
      ...
      if (json.UserId) {
        ...
        setData({ ...data, email: "", password: "", fullname:"" });
        navigation.navigate("Main",{userFullName:data.fullname});
      } else {
        Alert.alert(
          " ",
          "ERROR: " + json.Error,
  ...
  return (
    ...
    >
      ...
      <View style={[styles.action,{ marginTop: 20 }]}>
        <FontAwesome name="user-o" color="darkred" size={20} />
        <TextInput
          placeholder="John"
          placeholderTextColor="#666666"
          style={[
